...
function SubscribeContext(options)
{
  ...
  {
    var port     = options.reference_port     || 0
    var hostname = options.reference_hostname || '0.0.0.0'
    ...
    var server = createServer(function(req, res)
    {
      ...
      {
        res.end()
        processResponse.call(self, body.toString())
      }))
    })
    .listen(port, hostname, function()
    {
      ...
      requestSubscribe('http://localhost:'+address.port)
    })
  }
  ...
  function onCloseRequest(result)
  {
    ...
    if(server)
      server.close(self.push.bind(self, null))
    else
    {
      eventSource.close()
      self.push(null)
